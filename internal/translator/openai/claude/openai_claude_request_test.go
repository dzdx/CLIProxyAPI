...
func TestConvertClaudeRequestToOpenAI_DoesNotStripNonBillingSystemText(t *testing.T) {
	input := []byte(`{
		"model":"claude-3-opus",
		"system":[
			{"type":"text","text":"please keep literal cch=00000 in this message"}
		],
		"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]
	}`)

	out := ConvertClaudeRequestToOpenAI("gpt-test", input, false)
	if got := gjson.GetBytes(out, "messages.0.content.0.text").String(); got != "please keep literal cch=00000 in this message" {
		t.Fatalf("system content text = %q, want preserved non-billing text; output=%s", got, string(out))
	}
}
